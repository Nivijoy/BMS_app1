...
export class InvoicelistComponent implements OnInit {
  ...
  async refresh() {
    ...
    this.sername = ''; this.sub_plan = ''; this.inv_type = '';
    ...
    if (res) {
      ...
      for (var i = 0; i < temp.length; i++) {
        ...
        if (this.role.getroleid() > 111) {
          ...
          param['RESELLER BALANCE'] = temp[i]['r_after_balance'];
        ...
        //   param['SUB ISP AMOUNT'] = temp[i]['sub_isp_amt'];
        //   param['SUB DIST SHARE'] = temp[i]['sub_dist_share'] + " " + '%';
        //   param['SUB DIST AMOUNT'] = temp[i]['sub_dist_amt'];
      ...
      JSXLSX.writeFile(wb, 'Combo Invoice  List' + EXCEL_EXTENSION);
    }
  }
