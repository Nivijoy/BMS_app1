...
@Component({
  selector: 'ngx-invoice-mail-log',
  templateUrl: './invoice-mail-log.component.html',
  styleUrls: ['./invoice-mail-log.component.scss']
})
export class InvoiceMailLogComponent implements OnInit {
  ...
  async initiallist() {
    this.loading = true;
    let result = await this.account.listInvoice(
      {
  ...
  async download() {
    this.loading = true;
    let res = await this.account.listInvoice(
      {
        bus_id: this.bus_name,
        resel_id: this.res_name,
        uid: this.cust_name,
