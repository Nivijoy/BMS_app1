...
@Component({
  selector: 'listbandwidthlog',
  templateUrl: './listbandwidthlog.component.html',
  styleUrls: ['./listbandwidthlog.component.scss']
})

export class ListBandwidthLogComponent implements OnInit {
  ...
  async initiallist() {
    this.loading = true;
    let result = await this.repser.listBandwidthLog(
      {
  ...
  async download() {
    let res = await this.repser.listBandwidthLog({})
      if (res) {
        ...
        for (var i = 0; i < temp.length; i++) {
          ...
          param['RESELLER NAME'] = temp[i]['res_name'];
          param['SUB ISP NAME'] = temp[i]['subisp_name'];
          param['SUB DISTRIBUTOR NAME'] = temp[i]['subdist_name'];
          param['CREATED BY'] = temp[i]['cby_name'];
        ...
        JSXLSX.writeFile(wb, 'Bandwidth Log' + EXCEL_EXTENSION);
      }
  }
