...
import { AdminuserService, ResellerService, RoleService } from '../../_service/indexService';
...
export class AddUserprofileComponent implements OnInit {
  @ViewChild('tree') public tree; resell;
  AddProfileForm; id; roles = []; x = 0;
  submit: boolean;
  nodes = [
    {
      name: 'Dashboard',
  ...
  constructor(
    ...
    this.id = JSON.parse(localStorage.getItem('profile'));
    this.roles = JSON.parse(JSON.parse(localStorage.getItem('userinfo'))['menu_role']);
  }

  async ngOnInit() {
    // console.log('Initialize Node /n', JSON.stringify(this.nodes))
    this.createForm();
  ...
  async getprofile() {
    let result = await this.admser.getAdminProfileedit({ profile_id: this.id['id'] })
    if (result) {
  ...
  roleChecker() {
    // console.log('Rolecheck', JSON.parse(JSON.stringify(this.nodes)))
    let len = 0;
    for (const node of this.nodes) {
      this.removeRole(node.children);
      // this.removeNode(node);
      len++;
    }
    // console.log('length', len, this.nodes.length, this.nodes)
    // if (len == this.nodes.length) {
    //   for (const node of this.nodes) {
    //     // console.log('Rolecheckremove',JSON.parse(JSON.stringify(this.nodes)))
  ...
  removeRole(node) {
    if (node.children) {
      ...
      for (let i = 0; i < node.length;) {
        const child = node[i]
        if (child.children) {
          this.removeRole(child.children)
          i++
        } else if (!this.roles.find(x => x == child.id)) {
          node.splice(node.findIndex(n => n == child), 1)
        } else {
          i++
        }
      }
  ...
  removeNode(node): void {
    if (node.children.length == 0) {
      // node.parent.nodes.children.splice(node.parent.nodes.children.indexOf(node.nodes), 1);
      this.nodes.splice(this.x, 1);
      this.tree.treeModel.update();
    }
  }
  ...
  async AddProfile() {
    ...
    if (this.role.getroleid() <= 777 && this.id) {
      val['roleId'] = this.id['id'];
      val['Role'] = this.id['role']
    }
    if (this.role.getroleid() >= 777 && this.id) {
      val['roleId'] = this.id['id'];
      val['manager'] = this.id['managers_id']
      val['Role'] = this.id['role']
    }
    let result = await this.admser[method](val)
    this.toastalert(result['msg'], result['status']);
    if (result['status'] == 1) {
      this.router.navigate(['/pages/administration/userprofilelist'])
    }
  }
  ...
  createForm() {
    this.AddProfileForm = new FormGroup({
      menu_name: new FormControl(this.id ? this.id['menu_name'] : '', Validators.required),
      descr: new FormControl(this.id ? this.id['desc'] : ''),
    });
  }
}