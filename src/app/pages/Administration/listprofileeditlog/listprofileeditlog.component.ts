...
@Component({
  selector: 'listprofileeditlog',
  templateUrl: './listprofileeditlog.component.html',
  styleUrls: ['./listprofileeditlog.component.scss']
})

export class ListProfileEditLogComponent implements OnInit {
  ...
  async initiallist() {
    this.loading = true;
    let result = await this.repser.listEditProfileLog(
      {
  ...
  async download() {
    let res = await this.repser.listEditProfileLog({})
    if (res) {
      ...
      for (var i = 0; i < temp.length; i++) {
        ...
        param['RESELLER NAME'] = temp[i]['resel_name'];
      ...
      JSXLSX.writeFile(wb, 'Profile Edit Log' + EXCEL_EXTENSION);
    }
  }
